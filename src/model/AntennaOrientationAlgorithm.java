...
public class AntennaOrientationAlgorithm {
    ...
    public void setupOmniNet() {
        ...
        Quicksort<Link> qsortEdges = new Quicksort<Link>(omniNet.edges());
        List<Link> sortedEdges = qsortEdges.getSortedList();
        ...
        if (numEdges > 0) {
            edgeLen = sortedEdges.get(numEdges - 1).getWeight();
        }
    ...
    private void setupOmniNet(float sensorRange) {
        ...
        while (sensorsIterator.hasNext()) {
            ...
            this.averageOmniAngle =
                    (this.averageOmniAngle * averageIdx/(averageIdx + 1)) +
                    (sensor.getAntennaAngle() * 1/(averageIdx + 1));
            this.averageOmniRange =
                    (this.averageOmniRange * averageIdx/(averageIdx + 1)) +
                    (sensor.getAntennaRange() * 1/(averageIdx + 1));
            // Area = 1/2 r^2 angle.
            this.totalOmniEnergyUse +=
                    (0.5d * Math.pow(sensor.getAntennaRange(), 2) *
                            sensor.getAntennaAngle());
    ...
    public void setupDirNet() {
        ...
        // Our antenna range needs to be at least as large as
        // the longest edge. (Range is measured as radius)
        Quicksort<Link> qsortEdges = new Quicksort<Link>(dirNet.edges());
        List<Link> sortedEdges = qsortEdges.getSortedList();
        ...
        while (verticesIter.hasNext()) {
            ...
            while (edgesIter.hasNext()) {
                connectedVertices.add(
                        dirNet.opposite(sensor, edgesIter.next()));
            }
    ...
    // calculated.
    private void setSensorPropsDirNet(
            Sensor fromSensor, Set<Sensor> toSensors, float range) {
        ...
        while (toSensorsIter.hasNext()) {
            ...
            if (distance > longestRange) { longestRange = distance; }
        ...
        if (angles.size() > 0) {
            // Sort the angles.
            Quicksort<Float> qsortAngles = new Quicksort<Float>(angles);
            List<Float> sortedAngles = qsortAngles.getSortedList();
            ...
            for (int i = 0; i < sortedAngles.size(); i++) {
                ...
                if (lastAngleIdx < 0) {
                    lastAngleIdx = sortedAngles.size() + lastAngleIdx;
                }
                
                float tempAngle =
                        sortedAngles.get(i) - sortedAngles.get(lastAngleIdx);
            ...
            float angleCurrent = sortedAngles.get(largestAngleIdx);
            
            int lastAngleIdx = largestAngleIdx - 1;
            if (lastAngleIdx < 0) {
                lastAngleIdx = sortedAngles.size() + lastAngleIdx;
            }
            float angleLast = sortedAngles.get(lastAngleIdx);
        ...
        // Keep track of average angles & range
        this.averageDirAngle =
                (this.averageDirAngle * averageIdx/(averageIdx + 1)) +
                (fromSensor.getAntennaAngle() * 1/(averageIdx + 1));
        this.averageDirRange =
                (this.averageDirRange * averageIdx/(averageIdx + 1)) +
                (fromSensor.getAntennaRange() * 1/(averageIdx + 1));
        this.totalDirEnergyUse +=
                (0.5f * Math.pow(fromSensor.getAntennaRange(), 2) *
                        fromSensor.getAntennaAngle());
        averageIdx++;
    }
    ...
    private float getGraphDiameterLength(WeightedGraph<Sensor, Link> graph) {
        ...
        while (outer.hasNext()) {
            ...
            DijkstraSSSP<Sensor, Link> sssp =
                    new DijkstraSSSP<Sensor, Link>(graph, u);
    ...
    private float getShortestRouteLength(
            WeightedGraph<Sensor, Link> g, String u, String v) {
        ...
        while (vertsIter.hasNext()) {
            ...
            if (fromSensor != null && toSensor != null) { break; }
            if (current.getName().equals(u)) { fromSensor = current; }
            if (current.getName().equals(v)) { toSensor = current; }
        }
        
        if (fromSensor == null || toSensor == null) { return 0f; }
    ...
    private float getShortestRouteLength(
            WeightedGraph<Sensor, Link> g, Sensor u, Sensor v) {
    ...
    //of two nodes.
    private int getShortestRouteLengthHops(
            WeightedGraph<Sensor, Link> g, String u, String v) {
    ...
    private List<Sensor> getShortestRoute(
            WeightedGraph<Sensor, Link> g, String u, String v) {
    ...
    public WeightedGraph<Sensor, Link> getOmniNet() { return omniNet; }
    public WeightedGraph<Sensor, Link> getDirNet() { return dirNet; }
    public double getOmniAverageAngle() { return averageOmniAngle; }
    public double getOmniAverageRange() { return averageOmniRange; }
    public double getOmniTotalEnergyUse() { return totalOmniEnergyUse; }
    public double getDirAverageAngle() { return averageDirAngle; }
    public double getDirAverageRange() { return averageDirRange; }
    public double getDirTotalEnergyUse() { return totalDirEnergyUse; }
