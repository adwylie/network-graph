...
public class KruskalMST<V extends VertexInterface, E extends WeightedEdgeInterface> {
	...
	public KruskalMST(WeightedGraph<V, E> graph) {
		...
		List<E> edges = new ArrayList<E>(graph.edges());
		...
		for (int k = 0; k < edges.size(); k++) {
			...
			int i = vertexHash
					.get(edgeVerticesIter.hasNext() ? edgeVerticesIter.next()
							: -1);
			int j = vertexHash
					.get(edgeVerticesIter.hasNext() ? edgeVerticesIter.next()
							: -1);
			...
			if (i != j) {
				...
				// larger forest.
				Iterator<V> forestIterator = forests.get(smallForestIdx)
						.iterator();
				...
				forests.get(largeForestIdx).addAll(forests.get(smallForestIdx));
